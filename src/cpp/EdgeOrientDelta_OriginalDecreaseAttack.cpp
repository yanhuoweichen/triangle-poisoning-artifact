...
// ======================= Original-data poisoning attack settings =======================
// Edit these two values when you want to run different attack strengths.
static const double MALICIOUS_RATIO = 0.18;   // 恶意用户占全部用户的比例，例如 0.05 / 0.10 / 0.20
static const double POISON_PROB = 1.00;       // 恶意用户之间添加/删除边的概率，例如 0.1 / 0.3 / 0.5 / 1.0

// This file is generated for one fixed attack direction. Do not change this unless you know
// you want to reuse the same file for another attack type.
enum class OriginalAttackType { NONE, INCREASE, DECREASE };
static const OriginalAttackType ORIGINAL_ATTACK_TYPE = OriginalAttackType::DECREASE;
...
static const char* attackTypeName(OriginalAttackType t) {
    switch (t) {
        case OriginalAttackType::INCREASE: return "original_increase";
        case OriginalAttackType::DECREASE: return "original_decrease";
        default: return "none";
    }
}

static void rebuildAdjFromSet(Graph &g) {
    g.m = 0;
    g.adj.assign(g.n, {});
    for (int u = 0; u < g.n; ++u) {
        g.adj[u].reserve(g.adj_set[u].size());
        for (int v : g.adj_set[u]) {
            g.adj[u].push_back(v);
            if (u < v) g.m++;
        }
...
static AttackStats applyOriginalDataAttack(
    Graph &g,
    OriginalAttackType attack_type,
    double malicious_ratio,
    double poison_prob,
    RandomGen &rng
) {
    AttackStats stats;
    if (attack_type == OriginalAttackType::NONE) return stats;

    malicious_ratio = max(0.0, min(1.0, malicious_ratio));
    poison_prob = max(0.0, min(1.0, poison_prob));

    const int k = (int)floor((double)g.n * malicious_ratio);
    if (k <= 1 || poison_prob <= 0.0) return stats;

    vector<int> nodes(g.n);
    iota(nodes.begin(), nodes.end(), 0);

    // Randomly choose controlled real users. The seed comes from the program seed so
    // the same command is reproducible. You can later replace this with high-degree
    // selection if you want a stronger targeted attack.
    for (int i = g.n - 1; i > 0; --i) {
        int j = (int)floor(rng.uniform01() * (double)(i + 1));
        if (j < 0) j = 0;
        if (j > i) j = i;
        swap(nodes[i], nodes[j]);
    }

    vector<int> malicious(nodes.begin(), nodes.begin() + k);
    sort(malicious.begin(), malicious.end());
    stats.malicious_nodes = k;

    for (int a = 0; a < k; ++a) {
        int u = malicious[a];
        for (int b = a + 1; b < k; ++b) {
            int v = malicious[b];
            bool has_edge = g.adj_set[u].find(v) != g.adj_set[u].end();

            if (attack_type == OriginalAttackType::INCREASE) {
                if (!has_edge && rng.uniform01() < poison_prob) {
                    g.adj_set[u].insert(v);
                    g.adj_set[v].insert(u);
                    stats.added_edges++;
                }
            } else if (attack_type == OriginalAttackType::DECREASE) {
                if (has_edge && rng.uniform01() < poison_prob) {
                    g.adj_set[u].erase(v);
                    g.adj_set[v].erase(u);
                    stats.removed_edges++;
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        Timer loadTimer;
        Graph clean_g = readGraphAuto(edgeFile, nArg);
        double loadSec = loadTimer.elapsed();

        if (clean_g.n > 50000) {
            cerr << "Refusing to run n=" << clean_g.n << " because this version materializes a dense n*n RR matrix. "
                 << "Use n=10000/20000 for the thesis experiments or replace the RR matrix with a sparse/hash oracle.\n";
            return 2;
        }

        Timer cleanExactTimer;
        long long cleanExactTri = exactTriangleCount(clean_g);
        double cleanExactSec = cleanExactTimer.elapsed();

        Graph g = clean_g;
        RandomGen attackRng(makeSeed(baseSeed, 9090, 0));
        AttackStats attackStats = applyOriginalDataAttack(g, ORIGINAL_ATTACK_TYPE, MALICIOUS_RATIO, POISON_PROB, attackRng);

        Timer poisonedExactTimer;
        long long poisonedExactTri = exactTriangleCount(g);
        double poisonedExactSec = poisonedExactTimer.elapsed();

        ofstream out(outputFile.c_str());
        if (!out) throw runtime_error("Cannot open output file: " + outputFile);
        out << "edge_file,n_header,n_used,attack_type,malicious_ratio,poison_prob,malicious_nodes,added_edges,removed_edges,"
            << "clean_m_used,poisoned_m_used,clean_exact_triangles,poisoned_exact_triangles,"
            << "load_seconds,clean_exact_seconds,poisoned_exact_seconds,epsilon,psi,workers,runs,run,noise,seed,"
            << "split_factor,bias_factor,dmax_slack_multiplier,estimate,"
            << "abs_error_vs_clean,relative_error_vs_clean,abs_error_vs_poisoned,relative_error_vs_poisoned,"
        ...
        cerr << "Loaded " << edgeFile << " | header n=" << clean_g.n_header << " | used n=" << clean_g.n
             << " | clean_m=" << clean_g.m << " | poisoned_m=" << g.m
             << " | clean exact triangles=" << cleanExactTri
             << " | poisoned exact triangles=" << poisonedExactTri
             << " | attack=" << attackTypeName(ORIGINAL_ATTACK_TYPE)
             << " | malicious_ratio=" << MALICIOUS_RATIO
             << " | poison_prob=" << POISON_PROB
             << " | malicious_nodes=" << attackStats.malicious_nodes
             << " | added_edges=" << attackStats.added_edges
             << " | removed_edges=" << attackStats.removed_edges
        ...
        for (double eps : epsList) {
            ...
            for (int r = 0; r < runs; ++r) {
                ...
                double absErrClean = fabs(res.estimate - (double)cleanExactTri);
                double relErrClean = (cleanExactTri == 0) ? absErrClean : absErrClean / max(1.0, fabs((double)cleanExactTri));
                double absErrPoisoned = fabs(res.estimate - (double)poisonedExactTri);
                double relErrPoisoned = (poisonedExactTri == 0) ? absErrPoisoned : absErrPoisoned / max(1.0, fabs((double)poisonedExactTri));

                out << edgeFile << ',' << clean_g.n_header << ',' << clean_g.n << ','
                    << attackTypeName(ORIGINAL_ATTACK_TYPE) << ',' << MALICIOUS_RATIO << ',' << POISON_PROB << ','
                    << attackStats.malicious_nodes << ',' << attackStats.added_edges << ',' << attackStats.removed_edges << ','
                    << clean_g.m << ',' << g.m << ',' << cleanExactTri << ',' << poisonedExactTri << ','
                    << fixed << setprecision(8) << loadSec << ',' << cleanExactSec << ',' << poisonedExactSec << ','
                    << eps << ',' << psi << ',' << workers << ',' << runs << ',' << r << ',' << (use_noise ? 1 : 0) << ',' << seed << ','
                    << split_factor << ',' << bias_factor << ',' << dmax_slack_multiplier << ','
                    << res.estimate << ',' << absErrClean << ',' << relErrClean << ',' << absErrPoisoned << ',' << relErrPoisoned << ','
                ...
                cerr << "eps=" << eps << " run=" << r << " estimate=" << res.estimate
                     << " rel_error_vs_clean=" << relErrClean
                     << " rel_error_vs_poisoned=" << relErrPoisoned
                     << " dmax=" << res.max_noisy_out_degree
