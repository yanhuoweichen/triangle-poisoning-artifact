...
# EdgeOrient: choose one representative defense group.
...
mkdir -p "${OUT_DIR}"
SRC=EdgeOrient_AnomalyDefense.cpp
EXE=./EdgeOrient_AnomalyDefense
OUT=${OUT_DIR}/${DATASET}_EdgeOrient_rr_inc_defense.csv
...
for MR in ${RATIOS}; do
  echo "[Run] EdgeOrient attack=${ATTACK} eps=${EPS} ratio=${MR}"
  "${EXE}" --input "${EDGE_FILE}" --n "${NODE_NUM}" --protocol EdgeOrient \
