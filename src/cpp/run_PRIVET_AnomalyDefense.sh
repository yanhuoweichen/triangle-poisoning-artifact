...
# PRIVET: choose one representative defense group.
# Representative attack: post-Laplace local-count increase, because PRIVET uses Laplace-calibrated numerical reports.
DATASET=${1:-Gplus}
NODE_NUM=${2:-10000}
RUNS=${3:-3}
...
mkdir -p "${OUT_DIR}"
SRC=PRIVET_AnomalyDefense.cpp
EXE=./PRIVET_AnomalyDefense
OUT=${OUT_DIR}/${DATASET}_PRIVET_lap_local_inc_defense.csv

EPS=${EPS:-1.0}
RATIOS=${RATIOS:-"0 0.025 0.05 0.075 0.10"}
ATTACK=${ATTACK:-lap_local_inc}
ATTACK_STRENGTH=${ATTACK_STRENGTH:-0.10}
...
for MR in ${RATIOS}; do
  echo "[Run] PRIVET attack=${ATTACK} eps=${EPS} ratio=${MR}"
  "${EXE}" --input "${EDGE_FILE}" --n "${NODE_NUM}" --protocol PRIVET \
