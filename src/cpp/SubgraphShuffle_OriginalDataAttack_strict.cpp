...
// ===== ORIGINAL_DATA_ATTACK_GLOBALS_BEGIN =====
// Extra parameters for original-data poisoning experiments.
// MaliciousRatio: fraction of existing users controlled by the adversary.
// AttackDirection: 1 = increase attack, 0 = decrease attack.
double MaliciousRatio = 0.0;
int AttackDirection = 1;
vector<int> g_is_malicious;

static bool IsIncreaseDirection(const string &s){
	return s == "increase" || s == "inc" || s == "+" || s == "1" || s == "add";
}
// ===== ORIGINAL_DATA_ATTACK_GLOBALS_END =====
...
// Initialize malicious users deterministically for reproducibility.
// This function only marks controlled users; it does not modify the graph.
static void InitMaliciousUsersIfNeeded(){
	if(!g_is_malicious.empty()) return;
	g_is_malicious.assign(NodeNum, 0);

	int mal_num = (int)floor((double)NodeNum * MaliciousRatio + 1e-9);
	if(mal_num < 0) mal_num = 0;
	if(mal_num > NodeNum) mal_num = NodeNum;

	vector<int> ord(NodeNum);
	for(int i = 0; i < NodeNum; i++) ord[i] = i;

	// Use the C library RNG here so malicious-user selection does not consume
	// the Mersenne Twister sequence used by the original algorithm.
	srand(1776);
	for(int i = 0; i < mal_num; i++){
		int rnd = i + (rand() % (NodeNum - i));
		int tmp = ord[i];
		ord[i] = ord[rnd];
		ord[rnd] = tmp;
	}
	for(int i = 0; i < mal_num; i++) g_is_malicious[ord[i]] = 1;

	cerr << "[AttackConfig] original-data attack, malicious_ratio=" << MaliciousRatio
	     << ", malicious_users=" << mal_num << "/" << NodeNum
	     << ", direction=" << (AttackDirection == 1 ? "increase" : "decrease") << endl;
}

// Attack original graph data before any shuffle-DP randomization.
// Increase: for every pair of malicious users, force the original edge to 1.
// Decrease: for every pair of malicious users, force the original edge to 0.
static void ApplyOriginalDataAttack(map<int, int> *a_mat){
	if(MaliciousRatio <= 0.0) return;
	InitMaliciousUsersIfNeeded();

	long long candidate_pairs = 0;
	long long changed_edges = 0;
	for(int i = 0; i < NodeNum; i++){
		if(!g_is_malicious[i]) continue;
		for(int j = i + 1; j < NodeNum; j++){
			if(!g_is_malicious[j]) continue;
			candidate_pairs++;

			if(AttackDirection == 1){
				// Original-data increase attack: malicious-malicious edge is set to 1.
				if(a_mat[i].count(j) == 0){
					a_mat[i][j] = 1;
					a_mat[j][i] = 1;
					changed_edges++;
				}
				...
				// Original-data decrease attack: malicious-malicious edge is set to 0.
				if(a_mat[i].count(j) > 0){
					a_mat[i].erase(j);
					a_mat[j].erase(i);
					changed_edges++;
	...
	cerr << "[OriginalDataAttack] candidate_malicious_pairs=" << candidate_pairs
	     << ", changed_undirected_edges=" << changed_edges << endl;
}
...
int main(int argc, char *argv[])
{
	...
	if (argc < 2) {
	    printf("Usage: %s [EdgeFile (in)] ([#nodes (default: -1)] [epsilon-delta (default: 1-8)] [#pairs (default:-1)] [#itr(-1) (default: 1)] [alg (default: 0)] ([malicious_ratio direction] [bi (default: 0)]))]))\n\n", argv[0]);
		...
		printf("[malicious_ratio direction]: optional original-data attack, e.g., 0.05 increase or 0.10 decrease\n");
	...
	// Extra arguments for original-data poisoning experiments:
	//   argv[7] = malicious ratio, e.g., 0.05, 0.10, 0.15, 0.20
	//   argv[8] = direction: increase/decrease, inc/dec, +/-, 1/0
	// Optional bipartite flag is moved to argv[9] for this attack executable.
	if(argc >= 9){
		MaliciousRatio = atof(argv[7]);
		string dir_s = argv[8];
		AttackDirection = IsIncreaseDirection(dir_s) ? 1 : 0;
	}

	Bip = 0;
	if(argc >= 10) Bip = atoi(argv[9]);
	else if(argc == 8) Bip = atoi(argv[7]);
	...
	for(itr=0;itr<ItrNum;itr++){
		...
		if(NodeNum < all_node_num || itr == 0){
			...
			// This is deliberately placed before degree, #2-stars, and #tri(true) are computed,
			// so the reported true count reflects the attacked original graph.
			ApplyOriginalDataAttack(a_mat);
