...
void CalcShuffleTri(map<int, int> *a_mat, double &tri_num_ns, int *deg, int alg){
	...
	for(i=0;i<NodeNum;i+=2){
		...
		for(j=0;j<NodeNum;j++){
			...
			// ===== STRICT_SHUFFLE_ATTACK_BEGIN =====
			// Shuffle attack changes only shuffled wedge messages for fully malicious triangle candidates.
			// Edge RR reports edge12_ns/edge21_ns are intentionally not modified here.
			// Increase attack: force the malicious-internal wedge message to 1.
			// Decrease attack: force the malicious-internal wedge message to 0.
			if(MaliciousRatio > 0.0 && g_is_malicious[node1] && g_is_malicious[node2] && g_is_malicious[j]){
				wedge_ns = AttackDirection;
			}
			// ===== STRICT_SHUFFLE_ATTACK_END =====
