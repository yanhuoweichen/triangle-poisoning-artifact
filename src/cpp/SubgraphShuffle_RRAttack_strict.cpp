...
// ===== STRICT_MALICIOUS_ATTACK_GLOBALS_BEGIN =====
// Extra parameters for robustness experiments.
...
static void InitMaliciousUsersIfNeeded(){
	...
	// Deterministically choose malicious users for reproducibility.
	// This does not change the original graph; it only marks controlled users.
	...
	cerr << "[AttackConfig] malicious_ratio=" << MaliciousRatio
	     << ", malicious_users=" << mal_num << "/" << NodeNum
	     << ", direction=" << (AttackDirection == 1 ? "increase" : "decrease") << endl;
}
// ===== STRICT_MALICIOUS_ATTACK_GLOBALS_END =====
...
void CalcShuffleTri(map<int, int> *a_mat, double &tri_num_ns, int *deg, int alg){
	...
	// ===== STRICT_MALICIOUS_ATTACK_INIT_BEGIN =====
	InitMaliciousUsersIfNeeded();
	// ===== STRICT_MALICIOUS_ATTACK_INIT_END =====
	...
	for(i=0;i<NodeNum;i+=2){
		...
		// ===== STRICT_RR_ATTACK_BEGIN =====
		// RR attack only changes the randomized edge reports between two malicious users.
		// It does NOT change the original graph, and it does NOT touch malicious-normal edges.
		// Increase attack: force the malicious-internal edge RR reports to 1.
		// Decrease attack: force the malicious-internal edge RR reports to 0.
		if(MaliciousRatio > 0.0 && g_is_malicious[node1] && g_is_malicious[node2]){
			edge12_ns = AttackDirection;
			edge21_ns = AttackDirection;
		}
		// ===== STRICT_RR_ATTACK_END =====
...
int main(int argc, char *argv[])
{
	...
	if (argc < 2) {
	    printf("Usage: %s [EdgeFile (in)] ([#nodes (default: -1)] [epsilon-delta (default: 1-8)] [#pairs (default:-1)] [#itr(-1) (default: 1)] [alg (default: 0)] ([malicious_ratio direction] or [bi (default: 0)]))]))\n\n", argv[0]);
	...
	// ===== STRICT_MALICIOUS_ATTACK_ARG_PARSE_BEGIN =====
	// Extra arguments used by the robustness scripts:
	//   argv[7] = malicious ratio, e.g., 0.025, 0.05, ..., 0.20
	//   argv[8] = direction: increase/decrease, inc/dec, +/-, 1/0
	// These experiments do not use the optional bipartite flag, so argv[7] is ratio.
	if(argc >= 9){
		MaliciousRatio = atof(argv[7]);
		string dir_s = argv[8];
		AttackDirection = IsIncreaseDirection(dir_s) ? 1 : 0;
	}
	// ===== STRICT_MALICIOUS_ATTACK_ARG_PARSE_END =====
