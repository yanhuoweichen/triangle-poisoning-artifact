...
# DDP: choose one representative defense group.
# Representative attack: post-Laplace local-count increase, because DDP phase-2 uses numerical Laplace reports.
DATASET=${1:-Gplus}
NODE_NUM=${2:-10000}
RUNS=${3:-3}
...
mkdir -p "${OUT_DIR}"
SRC=DDP_AnomalyDefense.cpp
EXE=./DDP_AnomalyDefense
OUT=${OUT_DIR}/${DATASET}_DDP_lap_local_inc_defense.csv
...
for MR in ${RATIOS}; do
  echo "[Run] DDP attack=${ATTACK} eps=${EPS} ratio=${MR}"
  "${EXE}" --input "${EDGE_FILE}" --n "${NODE_NUM}" --protocol DDP \
