...
/******************** Original-data increase attack settings ********************/
/*
 * This attack only modifies edges whose two endpoints are both malicious users.
 * It is applied immediately after reading the original graph and before degree,
 * true triangle, and private estimates are calculated.
 *
 * Extra command-line arguments added after the original arguments:
 *   argv[9]  : malicious user ratio, e.g., 0.05
 *   argv[10] : poison probability for adding each missing malicious-malicious edge
 *   argv[11] : attack seed
 */
double OrigIncMaliciousRatio = 0.0;
double OrigIncPoisonProb = 1.0;
int OrigIncAttackSeed = 20220610;
string OrigIncRatio_s = "0";
string OrigIncPoison_s = "1";
long long OrigIncCandidatePairs = 0;
long long OrigIncExistingPairs = 0;
long long OrigIncAddedEdges = 0;
int OrigIncMaliciousUsers = 0;
...
string OrigIncSuffix() {
	if (OrigIncMaliciousRatio <= 0.0) return "";
	return "_origInc_r" + SanitizeForFilename(OrigIncRatio_s)
		+ "_p" + SanitizeForFilename(OrigIncPoison_s)
		+ "_seed" + to_string(OrigIncAttackSeed);
}

unsigned long long OrigIncNextRand(unsigned long long &state) {
	state ^= state << 7;
	state ^= state >> 9;
	state ^= state << 8;
	return state;
...
double OrigIncRand01(unsigned long long &state) {
	unsigned long long x = OrigIncNextRand(state);
...
long long CountUndirectedEdges(map<int, int> *a_mat) {
	...
	for (int i = 0; i < NodeNum; i++) {
		edge_num += (long long)a_mat[i].size();
	}
	return edge_num / 2;
}

// Strict original-data increase attack: add only missing edges among malicious users.
long long ApplyOrigIncreaseAttackStrict(map<int, int> *a_mat, int itr) {
	int i, j;
	OrigIncCandidatePairs = 0;
	OrigIncExistingPairs = 0;
	OrigIncAddedEdges = 0;
	OrigIncMaliciousUsers = (int)floor((double)NodeNum * OrigIncMaliciousRatio + 1e-12);
	if (OrigIncMaliciousUsers < 0) OrigIncMaliciousUsers = 0;
	if (OrigIncMaliciousUsers > NodeNum) OrigIncMaliciousUsers = NodeNum;
	if (OrigIncMaliciousUsers <= 1 || OrigIncPoisonProb <= 0.0) return 0;

	bool *is_malicious = new bool[NodeNum];
	int *perm = new int[NodeNum];
	int *mal_nodes = new int[OrigIncMaliciousUsers];

	for (i = 0; i < NodeNum; i++) {
		is_malicious[i] = false;
		perm[i] = i;
	}

	unsigned long long state = (unsigned long long)OrigIncAttackSeed
		^ ((unsigned long long)(itr + 1) * 0x9E3779B97F4A7C15ULL)
		^ ((unsigned long long)NodeNum * 0xBF58476D1CE4E5B9ULL);

	// Fisher-Yates shuffle using a private RNG, so the original protocol RNG is not disturbed.
	for (i = NodeNum - 1; i > 0; i--) {
		j = (int)(OrigIncNextRand(state) % (unsigned long long)(i + 1));
		int tmp = perm[i];
		perm[i] = perm[j];
		perm[j] = tmp;
	}

	for (i = 0; i < OrigIncMaliciousUsers; i++) {
		is_malicious[perm[i]] = true;
		mal_nodes[i] = perm[i];
	}

	for (i = 0; i < OrigIncMaliciousUsers; i++) {
		int u = mal_nodes[i];
		for (j = i + 1; j < OrigIncMaliciousUsers; j++) {
			int v = mal_nodes[j];
			OrigIncCandidatePairs++;
			if (a_mat[u].count(v) > 0) {
				OrigIncExistingPairs++;
				continue;
			}
			if (OrigIncPoisonProb >= 1.0 || OrigIncRand01(state) < OrigIncPoisonProb) {
				a_mat[u][v] = 1;
				a_mat[v][u] = 1;
				OrigIncAddedEdges++;
			}
...
int main(int argc, char *argv[])
{
	...
	int itr;
	int i, j, k, x;
	long long orig_inc_edges_before, orig_inc_edges_after, orig_inc_added_edges_this_itr;
	string outdir;
	string outfile;
	char s[1025], *str;
	char str_1[] = "1";
...
//	init_genrand(1);
	...
	if (argc < 2) {
		...
		printf("[origIncRatio]: Optional. Malicious user ratio for original-data increase attack, e.g., 0.05\n");
		printf("[origIncPoisonProb]: Optional. Probability of adding each missing malicious-malicious edge, default: 1.0\n");
		printf("[origIncSeed]: Optional. Attack seed, default: 20220610\n");
		return -1;
	}
	...
	// Optional strict original-data increase attack parameters
	if (argc >= 10) {
		OrigIncRatio_s = argv[9];
		OrigIncMaliciousRatio = atof(argv[9]);
	}
	if (argc >= 11) {
		OrigIncPoison_s = argv[10];
		OrigIncPoisonProb = atof(argv[10]);
	}
	if (argc >= 12) {
		OrigIncAttackSeed = atoi(argv[11]);
	}
	if (OrigIncMaliciousRatio < 0.0 || OrigIncMaliciousRatio > 1.0) {
		printf("Error: origIncRatio must be in [0,1]\n");
		exit(-1);
	}
	if (OrigIncPoisonProb < 0.0 || OrigIncPoisonProb > 1.0) {
		printf("Error: origIncPoisonProb must be in [0,1]\n");
		exit(-1);
	}
	...
	outdir = EdgeFile.substr(0, i+1);
	for(i=0;i<3;i++){
		if(fix_perm) outfile = outdir + "res_n" + to_string(NodeNum) + "_alg" + to_string(Alg) + "_eps" + Eps_s + "-" + Mu_s + "_ns" + to_string(NSType) + "_cl" + Clip_s + "_ba" + Balloc_s[0] + "-" + Balloc_s[1] + "_itr" + to_string(ItrNum) + "-1" + OrigIncSuffix() + ".csv";
		else outfile = outdir + "res_n" + to_string(NodeNum) + "_alg" + to_string(Alg) + "_eps" + Eps_s + "-" + Mu_s + "_ns" + to_string(NSType) + "_cl" + Clip_s + "_ba" + Balloc_s[0] + "-" + Balloc_s[1] + "_itr" + to_string(ItrNum) + OrigIncSuffix() + ".csv";
		fp = FileOpen(outfile, "w");
	...
	for(itr=0;itr<ItrNum;itr++){
		...
		if(NodeNum < all_node_num || itr == 0){
			...
			// Strict original-data increase attack.
			// It is applied before computing deg, deg_lower, true triangles, and all private estimates.
			orig_inc_edges_before = CountUndirectedEdges(a_mat);
			orig_inc_added_edges_this_itr = 0;
			if (OrigIncMaliciousRatio > 0.0) {
				orig_inc_added_edges_this_itr = ApplyOrigIncreaseAttackStrict(a_mat, itr);
			}
			orig_inc_edges_after = CountUndirectedEdges(a_mat);
			...
			if (OrigIncMaliciousRatio > 0.0) {
				printf("[OrigIncreaseAttackStrict] itr=%d ratio=%s malicious_users=%d poison_prob=%s candidate_pairs=%lld existing_pairs=%lld added_edges=%lld edges_before=%lld edges_after=%lld true_triangles_after=%lld\n",
					itr, OrigIncRatio_s.c_str(), OrigIncMaliciousUsers, OrigIncPoison_s.c_str(),
					OrigIncCandidatePairs, OrigIncExistingPairs, orig_inc_added_edges_this_itr,
					orig_inc_edges_before, orig_inc_edges_after, tri_num);
				fflush(stdout);
			}
			...
			if(fix_perm) outfile = outdir + "res_n" + to_string(NodeNum) + "_alg" + to_string(Alg) + "_eps" + Eps_s + "-" + Mu_s + "_ns" + to_string(NSType) + "_cl" + Clip_s + "_ba" + Balloc_s[0] + "-" + Balloc_s[1] + "_itr" + to_string(ItrNum) + "-1" + OrigIncSuffix() + "_4cycles.csv";
			else outfile = outdir + "res_n" + to_string(NodeNum) + "_alg" + to_string(Alg) + "_eps" + Eps_s + "-" + Mu_s + "_ns" + to_string(NSType) + "_cl" + Clip_s + "_ba" + Balloc_s[0] + "-" + Balloc_s[1] + "_itr" + to_string(ItrNum) + OrigIncSuffix() + "_4cycles.csv";
			fp = FileOpen(outfile, "w");
