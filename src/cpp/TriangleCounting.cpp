...
/******************** 新增开始：投毒攻击参数 ********************/
double MaliciousRatio = 0.05;   // 恶意用户比例 x
double PoisonProb = 1;       // 恶意用户之间强制报1的概率 z

bool *g_is_malicious = NULL;
/******************** 新增结束：投毒攻击参数 ********************/

/******************** 新增开始：初始化恶意用户 ********************/
void InitPoisonUsers() {
    if (g_is_malicious != NULL) return;

    g_is_malicious = new bool[NodeNum];
    int mal_cnt = (int)(NodeNum * MaliciousRatio);

    // 先标记前 mal_cnt 个用户为恶意用户
...
if (Alg == 2 || Alg == 3 || Alg == 4) {
    for(i=0; i<NodeNum; i++){
        ...
        for(j=i+1; j<NodeNum; j++){
            ...
            // 恶意用户之间，若原始边存在，则以 PoisonProb 的概率zengjia
            if (a_mat[i].count(j) == 0 && genrand_real2() < PoisonProb) {
                a_mat[i][j] = 1;
                a_mat[j][i] = 1;
