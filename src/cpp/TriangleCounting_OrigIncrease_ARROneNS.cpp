...
// -----------------------------------------------------------------------------
// Self-contained random generator compatible with the original mt19937ar calls.
// This avoids requiring mt19937ar.c / mt19937ar.h when compiling this one file.
// -----------------------------------------------------------------------------
static std::mt19937 g_mt_engine(1776);

void init_genrand(unsigned long s) {
    g_mt_engine.seed((uint32_t)s);
}

void init_by_array(unsigned long init_key[], int key_length) {
    std::vector<uint32_t> seeds;
    for (int i = 0; i < key_length; i++) seeds.push_back((uint32_t)init_key[i]);
    std::seed_seq seq(seeds.begin(), seeds.end());
    g_mt_engine.seed(seq);
}

unsigned long genrand_int32(void) {
    return (unsigned long)g_mt_engine();
}

double genrand_real2(void) {
    static std::uniform_real_distribution<double> dist(0.0, 1.0);
    return dist(g_mt_engine);
}

// -----------------------------------------------------------------------------
// Original-data increase attack: only add missing edges among malicious users.
// Default script uses poison_prob = 1.0, so malicious users become a clique.
// -----------------------------------------------------------------------------
bool OrigIncreaseAttackEnabled = false;
double OrigIncreaseRatio = 0.0;
double OrigIncreasePoisonProb = 1.0;
int OrigIncreaseSeed = 1776;
long long OrigIncreaseAddedEdges = 0;
long long OrigIncreaseEdgesBefore = 0;
long long OrigIncreaseEdgesAfter = 0;
long long OrigIncreaseCleanTriangles = 0;
long long OrigIncreaseForcedTriangles = -1;

static inline long long Comb2LL(long long x) {
    return (x >= 2) ? x * (x - 1) / 2 : 0;
}

static inline long long Comb3LL(long long x) {
    return (x >= 3) ? x * (x - 1) * (x - 2) / 6 : 0;
}

long long CountUndirectedEdges(map<int, int> *a_mat) {
    long long edge_num = 0;
    for (int i = 0; i < NodeNum; i++) edge_num += (long long)a_mat[i].size();
    return edge_num / 2;
}

string OrigIncreaseRatioTag() {
    int pct = (int)floor(OrigIncreaseRatio * 100.0 + 0.5);
    return "_origInc_r" + to_string(pct) + "pct";
}

long long CountCleanTrianglesAndCategories(map<int, int> *a_mat, const vector<char> &is_malicious,
                                           long long &tri_mmm, long long &tri_2m1b) {
        ...
        for (aitr = a_mat[i].begin(); aitr != a_mat[i].end(); aitr++) {
            ...
            for (aitr2 = a_mat[i].begin(); aitr2 != a_mat[i].end(); aitr2++) {
                ...
                if (a_mat[j].count(k) > 0) {
                    tri_total++;
                    int mal_cnt = (is_malicious[i] ? 1 : 0) + (is_malicious[j] ? 1 : 0) + (is_malicious[k] ? 1 : 0);
                    if (mal_cnt == 3) tri_mmm++;
                    else if (mal_cnt == 2) tri_2m1b++;
                }
            }
        }
    }
...
void ApplyOrigIncreaseAttackStrict(map<int, int> *a_mat, int itr) {
    OrigIncreaseAddedEdges = 0;
    OrigIncreaseEdgesBefore = CountUndirectedEdges(a_mat);
    OrigIncreaseForcedTriangles = -1;

    int mal_num = (int)floor((double)NodeNum * OrigIncreaseRatio + 1e-12);
    if (mal_num <= 1) {
        OrigIncreaseEdgesAfter = OrigIncreaseEdgesBefore;
        printf("[OrigIncreaseAttackStrict] ratio=%.6f malicious_users=%d/%d added_edges=0 edges_before=%lld edges_after=%lld\n",
               OrigIncreaseRatio, mal_num, NodeNum, OrigIncreaseEdgesBefore, OrigIncreaseEdgesAfter);
        return;
    }

    vector<int> order(NodeNum);
    for (int i = 0; i < NodeNum; i++) order[i] = i;
    std::mt19937 attack_rng((uint32_t)(OrigIncreaseSeed + itr * 1000003 + (int)floor(OrigIncreaseRatio * 1000000.0 + 0.5)));
    std::shuffle(order.begin(), order.end(), attack_rng);

    vector<char> is_malicious(NodeNum, 0);
    vector<int> malicious_nodes;
    malicious_nodes.reserve(mal_num);
    for (int i = 0; i < mal_num; i++) {
        is_malicious[order[i]] = 1;
        malicious_nodes.push_back(order[i]);
    }

    // Before adding edges, compute the clean true triangle count and the triangle categories
    // that can be affected by adding only malicious-malicious edges.
    long long clean_mmm = 0;
    long long clean_2m1b = 0;
    OrigIncreaseCleanTriangles = CountCleanTrianglesAndCategories(a_mat, is_malicious, clean_mmm, clean_2m1b);

    // When poison_prob = 1.0, malicious users become a clique. We can compute the attacked
    // true triangle count exactly without enumerating all triangles in the dense attacked graph.
    if (OrigIncreasePoisonProb >= 1.0 - 1e-12) {
        long long final_mmm = Comb3LL((long long)mal_num);
        long long final_2m1b = 0;
        for (int w = 0; w < NodeNum; w++) {
            if (is_malicious[w]) continue;
            long long c = 0;
            for (map<int, int>::iterator it = a_mat[w].begin(); it != a_mat[w].end(); it++) {
                if (is_malicious[it->first]) c++;
            }
            final_2m1b += Comb2LL(c);
        }
        OrigIncreaseForcedTriangles = OrigIncreaseCleanTriangles + (final_mmm - clean_mmm) + (final_2m1b - clean_2m1b);
    }

    std::uniform_real_distribution<double> dist(0.0, 1.0);
    for (int a = 0; a < mal_num; a++) {
        int u = malicious_nodes[a];
        for (int b = a + 1; b < mal_num; b++) {
            int v = malicious_nodes[b];
            if (a_mat[u].count(v) == 0 && dist(attack_rng) < OrigIncreasePoisonProb) {
                a_mat[u][v] = 1;
                a_mat[v][u] = 1;
                OrigIncreaseAddedEdges++;
            }
        }
    }

    OrigIncreaseEdgesAfter = CountUndirectedEdges(a_mat);
    printf("[OrigIncreaseAttackStrict] ratio=%.6f poison_prob=%.6f malicious_users=%d/%d added_edges=%lld edges_before=%lld edges_after=%lld clean_triangles=%lld attacked_true_triangles=%lld\n",
           OrigIncreaseRatio, OrigIncreasePoisonProb, mal_num, NodeNum, OrigIncreaseAddedEdges,
           OrigIncreaseEdgesBefore, OrigIncreaseEdgesAfter, OrigIncreaseCleanTriangles,
           OrigIncreaseForcedTriangles);
}
...
//	init_genrand(1);
	...
	if (argc < 2) {
		...
		printf("[origIncRatio poisonProb seed]: optional original-data increase attack parameters; use alg=3 for ARROneNS.\n");
		return -1;
	}
	...
	// Optional original-data increase attack parameters.
	// argv[9]  = malicious ratio, e.g., 0.05
	// argv[10] = poison probability, default 1.0
	// argv[11] = attack seed, default 1776
	if (argc >= 10){
		OrigIncreaseRatio = atof(argv[9]);
		OrigIncreaseAttackEnabled = (OrigIncreaseRatio > 0.0);
	}
	if (argc >= 11) OrigIncreasePoisonProb = atof(argv[10]);
	if (argc >= 12) OrigIncreaseSeed = atoi(argv[11]);
	if (OrigIncreaseAttackEnabled){
		if (OrigIncreaseRatio < 0.0 || OrigIncreaseRatio > 1.0){
			printf("Error: incorrect original-data increase malicious ratio\n");
			exit(-1);
		}
		if (OrigIncreasePoisonProb < 0.0 || OrigIncreasePoisonProb > 1.0){
			printf("Error: incorrect original-data increase poison probability\n");
			exit(-1);
		}
	}
	...
	i = EdgeFile.find_last_of("/");
	outdir = EdgeFile.substr(0, i+1);
	if(OrigIncreaseAttackEnabled) attack_tag = OrigIncreaseRatioTag();
	else attack_tag = "";
	for(i=0;i<3;i++){
		if(fix_perm) outfile = outdir + "res_n" + to_string(NodeNum) + "_alg" + to_string(Alg) + "_eps" + Eps_s + "-" + Mu_s + "_ns" + to_string(NSType) + "_cl" + Clip_s + "_ba" + Balloc_s[0] + "-" + Balloc_s[1] + "_itr" + to_string(ItrNum) + "-1" + attack_tag + ".csv";
		else outfile = outdir + "res_n" + to_string(NodeNum) + "_alg" + to_string(Alg) + "_eps" + Eps_s + "-" + Mu_s + "_ns" + to_string(NSType) + "_cl" + Clip_s + "_ba" + Balloc_s[0] + "-" + Balloc_s[1] + "_itr" + to_string(ItrNum) + attack_tag + ".csv";
		fp = FileOpen(outfile, "w");
	...
	for(itr=0;itr<ItrNum;itr++){
		...
		if(NodeNum < all_node_num || itr == 0){
			...
			// Original-data increase attack: only add missing edges among malicious users.
			if(OrigIncreaseAttackEnabled) ApplyOrigIncreaseAttackStrict(a_mat, itr);
			...
			// #triangles --> tri_num
			// For poison_prob = 1.0, the attack code has already computed the exact attacked
			// true triangle count using the clean graph plus a closed-form delta. This avoids
			// enumerating all triangles in the dense malicious clique.
			if(OrigIncreaseAttackEnabled && OrigIncreaseForcedTriangles >= 0){
				tri_num = OrigIncreaseForcedTriangles;
			}
			else{
				tri_num = 0;
				for(i=0;i<NodeNum;i++){
			...
			if(fix_perm) outfile = outdir + "res_n" + to_string(NodeNum) + "_alg" + to_string(Alg) + "_eps" + Eps_s + "-" + Mu_s + "_ns" + to_string(NSType) + "_cl" + Clip_s + "_ba" + Balloc_s[0] + "-" + Balloc_s[1] + "_itr" + to_string(ItrNum) + "-1" + attack_tag + "_4cycles.csv";
			else outfile = outdir + "res_n" + to_string(NodeNum) + "_alg" + to_string(Alg) + "_eps" + Eps_s + "-" + Mu_s + "_ns" + to_string(NSType) + "_cl" + Clip_s + "_ba" + Balloc_s[0] + "-" + Balloc_s[1] + "_itr" + to_string(ItrNum) + attack_tag + "_4cycles.csv";
			fp = FileOpen(outfile, "w");
