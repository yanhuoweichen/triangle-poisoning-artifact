...
double MaliciousRatio = 0.05;     // 恶意用户比例（10%的用户是恶意的）
double PoisonProb = 1.0;         // 投毒概率（恶意用户以100%的概率报1）
bool *g_is_malicious = NULL;     // 全局数组，标记每个用户是否是恶意用户
...
// 步骤2：投毒（如果node1是恶意用户）
if(g_is_malicious != NULL && g_is_malicious[node1]) {
    if(genrand_real2() < PoisonProb) {
        edge12_ns = 0;  // 恶意用户强制报1
    }
}
...
// 步骤2：投毒（如果node2是恶意用户）
if(g_is_malicious != NULL && g_is_malicious[node2]) {
    if(genrand_real2() < PoisonProb) {
        edge21_ns = 0;  // 恶意用户强制报1
    }
}
...
// 投毒：恶意用户强制报1（增加4-cycle）
if(g_is_malicious != NULL && g_is_malicious[j] && genrand_real2() < PoisonProb) {
    wedge_ns = 0;  // 强制报1 → 4-cycle增加
}
...
// ========== 投毒攻击初始化（只对shuffle算法生效）==========
if (Alg == 2 || Alg == 3 || Alg == 4 || Alg == 8) {  // 三角形计数的shuffle算法
    g_is_malicious = new bool[NodeNum];
    int mal_cnt = (int)(NodeNum * MaliciousRatio);
    
    // 标记前 mal_cnt 个用户为恶意
    for(int i=0; i<NodeNum; i++) {
        g_is_malicious[i] = (i < mal_cnt);
    }
    
    // 随机打乱顺序
    for(int i=0; i<NodeNum; i++) {
        int r = genrand_int32() % NodeNum;
        bool tmp = g_is_malicious[i];
        g_is_malicious[i] = g_is_malicious[r];
        g_is_malicious[r] = tmp;
    }
    
    cout << "=== Edge Poisoning Attack ===" << endl;
    cout << "Malicious ratio: " << MaliciousRatio*100 << "%" << endl;
    cout << "Poison probability: " << PoisonProb << endl;
    cout << "==============================" << endl;
}
