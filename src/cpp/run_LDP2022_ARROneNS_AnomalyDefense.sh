...
# 2022 LDP / ARROneNS: choose one representative defense group.
# Representative attack: original-data increase among malicious users.
DATASET=${1:-Gplus}
NODE_NUM=${2:-10000}
RUNS=${3:-3}
...
mkdir -p "${OUT_DIR}"
SRC=LDP2022_ARROneNS_AnomalyDefense.cpp
EXE=./LDP2022_ARROneNS_AnomalyDefense
OUT=${OUT_DIR}/${DATASET}_LDP2022_ARROneNS_orig_inc_defense.csv

EPS=${EPS:-1.0}
RATIOS=${RATIOS:-"0 0.025 0.05 0.075 0.10"}
ATTACK=${ATTACK:-orig_inc}
...
for MR in ${RATIOS}; do
  echo "[Run] LDP_ARROneNS attack=${ATTACK} eps=${EPS} ratio=${MR}"
  "${EXE}" --input "${EDGE_FILE}" --n "${NODE_NUM}" --protocol LDP_ARROneNS \
