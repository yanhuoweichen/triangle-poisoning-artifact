...
static inline bool attackIsDecrease(const LaplaceAttackConfig &cfg) {
    string d = lowerStr(cfg.direction);
    return (d == "decrease" || d == "dec" || d == "-");
}

static inline bool attackEnabled(const LaplaceAttackConfig &cfg) {
    string d = lowerStr(cfg.direction);
    return (d == "increase" || d == "inc" || d == "+" || d == "decrease" || d == "dec" || d == "-")
           && cfg.malicious_ratio > 0.0 && cfg.poison_prob > 0.0 && cfg.value >= 0.0;
}

static vector<uint8_t> chooseMaliciousNodes(const Graph &g, const LaplaceAttackConfig &cfg, RandomGen &attack_rng) {
    int n = g.n;
...
static double computeLaplaceAttackOffset(
    const LaplaceAttackConfig &cfg,
    double laplace_noise_scale,
    double local_before_laplace,
    double local_after_laplace
) {
    (void)local_after_laplace;
    string mode = lowerStr(cfg.mode);
    double delta = 0.0;
    if (mode == "fixed" || mode == "constant" || mode == "const") {
        delta = cfg.value;
    } else if (mode == "scale" || mode == "noise_scale" || mode == "laplace_scale") {
        delta = cfg.value * max(0.0, laplace_noise_scale);
    } else if (mode == "local" || mode == "local_count" || mode == "count") {
        delta = cfg.value * max(1.0, fabs(local_before_laplace));
    }
    if (!isfinite(delta) || delta < 0.0) delta = 0.0;
    if (attackIsDecrease(cfg)) delta = -delta;
    return delta;
}
...
static vector<uint8_t> randomizedResponseNoisyMatrix(const Graph &g, double epsilon_rr, RandomGen &rng) {
    int n = g.n;
...
struct EdgeOrientResult {
    ...
    long long laplace_attack_candidates = 0;
    long long laplace_attack_reports = 0;
    double laplace_noise_scale = 0.0;
    double laplace_total_offset = 0.0;
    double laplace_abs_total_offset = 0.0;
    double malicious_local_pre_laplace_sum = 0.0;
...
static EdgeOrientResult runEdgeOrientDelta(
    ...
    int bias_factor,
    uint64_t seed,
    const LaplaceAttackConfig &attack_cfg
) {
    ...
    vector<uint8_t> X = use_noise ? randomizedResponseNoisyMatrix(g, eps_rr, rng)
                                  : vector<uint8_t>((size_t)g.n * (size_t)g.n, 0);
    if (!use_noise) {
        for (int u = 0; u < g.n; ++u) {
    ...
    for (int v = 0; v < g.n; ++v) {
        ...
        double local_before_laplace = local;
        ...
        res.laplace_noise_scale = lap_scale;

        // Laplace-stage poisoning attack: malicious controlled existing users tamper with
        // their locally noised triangle-count report before it reaches the coordinator.
        if (attackEnabled(attack_cfg) && is_malicious[v]) {
            res.laplace_attack_candidates++;
            res.malicious_local_pre_laplace_sum += local_before_laplace;
            if (attack_rng.uniform01() < attack_cfg.poison_prob) {
                double delta = computeLaplaceAttackOffset(attack_cfg, lap_scale, local_before_laplace, local);
                local += delta;
                res.laplace_attack_reports++;
                res.laplace_total_offset += delta;
                res.laplace_abs_total_offset += fabs(delta);
            }
        }
        totalCount += local;
    }
...
static void printUsage(const char *prog) {
    cerr << "Usage:\n"
         ...
         << "<attack_direction:none|increase|decrease> <attack_mode:fixed|scale|local> <malicious_ratio_list> <poison_prob> <selector:random|high_degree|low_degree> <attack_value> "
         << "[split_factor=0.5] [bias_factor=8]\n\n"
         << "Examples:\n"
         << "  " << prog << " ../data/Gplus/edges.csv 10000 1.0 0.5 2 5 1 1776 out.csv increase fixed 0.05,0.10 1.0 high_degree 10000\n"
         << "  " << prog << " ../data/Gplus/edges.csv 10000 1.0 0.5 2 5 1 1776 out.csv decrease scale 0.05,0.10 1.0 high_degree 1.0\n"
         << "  " << prog << " ../data/Gplus/edges.csv 10000 1.0 0.5 2 5 1 1776 out.csv increase local 0.05,0.10 1.0 high_degree 0.5\n";
}
...
int main(int argc, char **argv) {
    ...
    string attack_mode = argv[11];
    vector<double> maliciousRatioList = parseDoubleList(argv[12]);
    double poison_prob = atof(argv[13]);
    string malicious_selector = argv[14];
    double attack_value = atof(argv[15]);
    double split_factor = (argc >= 17) ? atof(argv[16]) : 0.5;
    int bias_factor = (argc >= 18) ? atoi(argv[17]) : 8;
    ...
    if (attack_value < 0.0) attack_value = 0.0;
    ...
    out << "edge_file,n_header,n_used,m_used,exact_triangles,load_seconds,exact_seconds,epsilon,psi,workers,runs,run,noise,seed,"
        << "laplace_attack_direction,laplace_attack_mode,malicious_ratio,poison_prob,malicious_selector,attack_value,malicious_nodes,"
        << "laplace_attack_candidates,laplace_attack_reports,laplace_noise_scale,laplace_total_offset,laplace_abs_total_offset,malicious_local_pre_laplace_sum,"
        ...
        for (double malRatioRaw : maliciousRatioList) {
            ...
            for (int r = 0; r < runs; ++r) {
                ...
                out << edgeFile << ',' << g.n_header << ',' << g.n << ',' << g.m << ',' << exactTri << ','
                    ...
                    << attack_direction << ',' << attack_mode << ',' << malRatio << ',' << poison_prob << ',' << malicious_selector << ',' << attack_value << ',' << res.malicious_nodes << ','
                    << res.laplace_attack_candidates << ',' << res.laplace_attack_reports << ',' << res.laplace_noise_scale << ','
                    << res.laplace_total_offset << ',' << res.laplace_abs_total_offset << ',' << res.malicious_local_pre_laplace_sum << ','
                ...
                cerr << "eps=" << eps << " mal=" << malRatio << " run=" << r
                     << " direction=" << attack_direction << " mode=" << attack_mode
                     << " reports=" << res.laplace_attack_reports
                     << " total_offset=" << res.laplace_total_offset
