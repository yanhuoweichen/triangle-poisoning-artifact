...
# TDP-VC: choose one representative defense group.
...
mkdir -p "${OUT_DIR}"
SRC=TDPVC_AnomalyDefense.cpp
EXE=./TDPVC_AnomalyDefense
OUT=${OUT_DIR}/${DATASET}_TDPVC_rr_inc_defense.csv
...
for MR in ${RATIOS}; do
  echo "[Run] TDP_VC attack=${ATTACK} eps=${EPS} ratio=${MR}"
  "${EXE}" --input "${EDGE_FILE}" --n "${NODE_NUM}" --protocol TDP_VC \
