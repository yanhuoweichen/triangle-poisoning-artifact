...
static EstimateResult EstimateTriangleCCS25(const Graph &g,
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = i + 1; j < n; ++j) {
            ...
            // RR attack is restricted to reports for edges whose two endpoints are both malicious.
            // In Radj, the upper-triangular edge {i,j} is reported by node i (i < j),
            // so the malicious-pair condition is mal[i] && mal[j].
            if ((args.attack_type == "rr_increase" || args.attack_type == "rr_decrease") && mal[i] && mal[j]) {
